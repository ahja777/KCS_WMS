...
export default function ContainerGroupsPage() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      addToast({ type: "success", message: `"${deletingGroup.groupName}" 용기군이 삭제되었습니다.` });
    } catch {
  ...
  const columns: Column<ContainerGroup>[] = [
    ...
    },
    {
      ...
      render: (row) => (
        <button
          onClick={(e) => { e.stopPropagation(); setDeletingGroup(row); }}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl bg-white p-6 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
        <div className="flex flex-wrap items-end gap-4">
          <div className="min-w-[200px] flex-1">
            <label className="mb-1 block text-sm font-medium text-[#4E5968]">물류용기군ID</label>
            <input type="text" placeholder="물류용기군ID" value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className={inputBase} />
          </div>
          <div className="min-w-[200px] flex-1">
            <label className="mb-1 block text-sm font-medium text-[#4E5968]">물류용기명</label>
            <input type="text" placeholder="물류용기명" className={inputBase} />
          </div>
      ...
      <div className="flex justify-end gap-2">
        <button onClick={handleCreate} className="rounded-xl bg-[#3182F6] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#1B64DA]">신규</button>
        <button onClick={() => addToast({ type: "info", message: "저장은 신규/수정 모달에서 진행해주세요." })} className="rounded-xl bg-[#F04452] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#D63341]">저장</button>
        ...
        <button onClick={() => { }} className="rounded-xl bg-[#8B95A1] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#6B7684]">삭제</button>
      </div>
      ...
      <div className="rounded-2xl bg-white p-6 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
        {error ? (
          ...
          <Table columns={columns} data={groups} isLoading={isLoading} page={page} totalPages={totalPages} total={total} onPageChange={setPage} onRowClick={handleEdit} emptyMessage="물류용기군 데이터가 없습니다." />
        )}
      </div>
      ...
      <ConfirmModal
        ...
        title="용기군 삭제"
        message={`"${deletingGroup?.groupName}" 용기군을 삭제하시겠습니까?`}
        confirmText="삭제"
