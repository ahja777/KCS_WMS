...
import { useOutboundOrders } from "@/hooks/useApi";
...
const columns: Column<OutboundOrder>[] = [
  ...
  {
    key: "orderedQtySum" as any,
    header: "주문수량",
    render: (row) => {
      const items = (row as any).items ?? row.lines;
      return items?.reduce((sum: number, i: any) => sum + (i.orderedQty || 0), 0) ?? 0;
    },
  },
  {
    key: "shippingMethod",
    header: "출하방법",
    render: (row) => row.shippingMethod || <span className="text-[#8B95A1]">-</span>,
  },
  {
    key: "shipDate",
    header: "출고일",
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[#191F28]">출고 관리</h1>
        <PageActions
          ...
          onExcelDownload={() => {
            ...
            downloadExcel(`/export/outbound${params}`, `출고목록_${new Date().toISOString().slice(0, 10)}.xlsx`);
          }}
          onPdfPrint={() => printPDF("출고 관리")}
          createLabel="출고 등록"
        />
      </div>
          ...
          {statusSteps.map((step, index) => (
            <div key={step.value} className="flex items-center">
              ...
              {index < statusSteps.length - 1 && (
                <div className="mx-2 h-[2px] w-8 bg-[#E5E8EB] lg:w-16" />
              )}
            </div>
          ))}
        </div>
