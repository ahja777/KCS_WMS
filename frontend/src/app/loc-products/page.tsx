    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-end gap-4">
          <div className="min-w-[160px]">
            ...
            <div className="flex gap-1">
              <select value={warehouseId} onChange={(e) => { setWarehouseId(e.target.value); setPage(1); }} className={`flex-1 ${inputBase}`}>
          ...
          <div className="min-w-[160px]">
            ...
            <div className="flex gap-1">
              <select value={partnerId} onChange={(e) => setPartnerId(e.target.value)} className={`flex-1 ${inputBase}`}>
                <option value="">전체</option>
          ...
          <div className="min-w-[160px]">
            ...
            <div className="flex gap-1">
              <input type="text" placeholder="상품" value={itemSearch} onChange={(e) => setItemSearch(e.target.value)} className={`flex-1 ${inputBase}`} />
              <button className="rounded-lg bg-[#F2F4F6] px-3 py-2 text-[#4E5968] hover:bg-[#E5E8EB]">
                <Search className="h-4 w-4" />
              </button>
            </div>
      ...
      <div className="flex flex-wrap items-start gap-6">
        ...
        <div className="flex gap-2">
          <button onClick={handleRegister} className="rounded-xl bg-[#F04452] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#D63341]">저장</button>
          <button className="rounded-xl bg-[#8B95A1] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#6B7684]">삭제</button>
          <button onClick={() => downloadExcel("/export/loc-products", "loc_products.xlsx")} className="rounded-xl bg-[#1FC47D] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#17A86B]">엑셀</button>
        </div>
          ...
          <div className="rounded-2xl border border-[#E5E8EB] bg-white p-4">
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                <label className="w-20 text-sm font-medium text-red-500">*로케이션</label>
                <div className="flex gap-1">
                  <input type="text" value={formLocation} onChange={(e) => setFormLocation(e.target.value)} className={`w-40 ${inputBase}`} />
                  <button className="rounded-lg bg-[#F2F4F6] px-2 py-2 text-[#4E5968] hover:bg-[#E5E8EB]">
                    <Search className="h-4 w-4" />
                  </button>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <label className="w-20 text-sm font-medium text-[#4E5968]">화주</label>
                <div className="flex gap-1">
                  <select value={formPartner} onChange={(e) => setFormPartner(e.target.value)} className={`w-40 ${inputBase}`}>
              ...
              <div className="flex items-center gap-3">
                <label className="w-20 text-sm font-medium text-red-500">*상품</label>
                <div className="flex gap-1">
                  <select value={formItem} onChange={(e) => setFormItem(e.target.value)} className={`w-40 ${inputBase}`}>
                    <option value="">선택</option>
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button onClick={handleReset} className="rounded-lg bg-[#F2F4F6] px-4 py-2 text-sm font-medium text-[#4E5968] hover:bg-[#E5E8EB]">초기화</button>
                <button onClick={handleRegister} className="rounded-lg bg-[#4E5968] px-4 py-2 text-sm font-medium text-white hover:bg-[#333D4B]">입고상품등록</button>
              </div>
            </div>
          </div>
        </div>
