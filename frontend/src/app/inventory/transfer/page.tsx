...
import { useWarehouses, useStockTransfer, useInventoryList } from "@/hooks/useApi";
...
export default function TransferPage() {
  ...
  // Search filters matching slide 35
  const { data: inventoryResponse, isLoading } = useInventoryList({ page, limit: 40 });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[#191F28]">재고이동등록</h1>
      </div>

      <InventoryTabNav />

      {/* Search area matching slide 35 */}
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button variant="secondary" size="sm" onClick={() => addToast({ type: "info", message: "닫기" })}>닫기</Button>
      </div>

      {/* Grid matching slide 35 */}
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-[#F7F8FA]">
              <tr>
                ...
                <SortableHeader field="updatedAt" sortKey={sortKey} sortDir={sortDir} onSort={handleSort}>작업시간</SortableHeader>
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr><td colSpan={8} className="py-16 text-center text-sm text-[#8B95A1]">데이터가 없습니다.</td></tr>
              ) : (
                sortedItems.map((item, idx) => (
                  <tr key={item.id ?? idx} className="cursor-pointer border-b border-[#F2F4F6] hover:bg-[#F7F8FA]">
                    <td className="px-3 py-3 text-center"><input type="checkbox" className="h-4 w-4 rounded border-[#D1D6DB]" /></td>
                    <td className="px-3 py-3 text-sm text-[#4E5968]">완료</td>
                    <td className="px-3 py-3 text-sm text-[#4E5968]">-</td>
                    <td className="px-3 py-3 text-sm font-mono text-[#4E5968]">{item.locationCode ?? "-"}</td>
